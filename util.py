...
def save_points(points, file_base_name, format="npy"):
    if format == "npy":
        np.save("output/" + file_base_name + ".npy", points)
    elif format == "txt":
        with open("output/" + file_base_name + ".txt", "w") as f:
            for p in points:
                f.write(f'{p[0]},{p[1]},{p[2]}\n')
    elif format == "pp":
        with open("output/" + file_base_name + ".pp", "w") as f:
            t = datetime.now()
            f.write(
        ...
        with open("output/" + file_base_name + ".obj", "w") as f:
            for p in points:
